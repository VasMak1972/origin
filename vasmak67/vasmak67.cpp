// vasmak67.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
...
void progressBar(Data& lhs, Data& rhs) {
    ...
    for (int i = 0; i < lhs.data.size(); ++i) {
        ...
        try {
            ...
            rhs.data.at(i) = tmp;
            bar.push_back(std::pair<std::string, int>(std::string(fill - previousFill, fillingChar), 2));
        }
        catch (const std::exception& e) {
            bar.push_back(std::pair<std::string, int>(std::string(fill - previousFill, fillingChar), 4));
        }
...
{
    std::scoped_lock<std::mutex, std::mutex> s_lock(lhs.m, rhs.m);
    std::cout << "Thread id: " << std::this_thread::get_id() << '\n';
    progressBar(lhs, rhs);
}
...
{
    std::unique_lock<std::mutex> ul1(lhs.m);
